...
| D-020 | 2026-08-06 | Aceptada | H3.1 mantiene `baseline_v1` como default y califica `optimized_v1` como candidata opt-in | El benchmark atribuye la mejora material a selección relevance-first; overlap y duplicación explican solo `0.277%` del prompt observado | `optimized_v1` requiere `input_token_budget_est`; su promoción necesita una decisión posterior explícita |
...
| D-022 | 2026-08-06 | Aceptada | Diferir `trim_overlap_v1` y conservar el diagnóstico report-only | Los `27` caracteres o `7` tokens estimados de overlap medidos son marginales frente a la pérdida de cobertura causada por la selección | H3.1 no manipula contenido ni rangos por overlap; se reevaluará únicamente ante evidencia reproducible y material |
